...
quant = np.vectorize(q)
# def quantize(tensor):
#     return [quant(i) for i in tensor]
...
def load_net(data_path):
    ...
    for name in weights_raw:
        ...
        if name[0:2] != '__':
            ...
            # kernels = quant(kernels); bias = quant(bias) ##################using quantization
...
def fire_cluster(net, x, preloaded, cluster_name,yy=[]):
    # central - squeeze
    layer_name = cluster_name + '/squeeze1x1'
    if cluster_name == 'fire4':
        # print(np.moveaxis(weights,-1,0).flatten()[0:10])
        # print(biases.flatten()[0:10])
        # print(x)
        yy.append(x)
...
def main():
    ...
    with g.as_default(), tf.Session(config=config) as sess:
        ...
        # Classifying
        ####################################
        sqznet_results = sqznet['yy'].eval(feed_dict={image: [preprocess(img_content, sqz_mean)], keep_prob: 1.})
        f=open("tmp.txt","w");[f.write("%.2f\n"%round(i,2)) for i in sqznet_results.flatten()[0:1000]]
        # exit()
